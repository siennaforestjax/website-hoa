...
router.get('/specialAlert', (req, res) => {
  res.send(
    'Next HOA Meeting is on October 27th 7pm EST at 2727 DownTheStreetLane'
  );
});
...
router.get('/documents/:category', (req, res) => {
  ...
  console.log(
    `readying this folder ${path.join(
      __dirname,
      'public',
      'documents',
      category
  ...
  fs.readdir(
    path.join(__dirname, 'public', 'documents', category),
    (err, files) => {
      if (err) {
        console.log(err);
        res.status(500).send({
        ...
        files.forEach((filename) => {
          const details = getFileDetails(
            path.join(__dirname, 'public', 'documents', category, filename),
            category
          );
